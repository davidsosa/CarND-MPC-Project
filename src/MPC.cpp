...
size_t N = 10;
double dt = 0.1;
// 0.2,0.5 - car too careful, lots of unjustified breaking 
// 0.05 lots of jitter, too unstable, would be                      
...
// set starting points for each state variable
size_t x_start = 0;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Change in steering and acceleration for a smoother ride
    for (int t = 0; t < N-2; t++) {
      fg[0] += weight_delta_change*pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
      fg[0] += weight_acc_change*pow(vars[a_start + t + 1] - vars[a_start + t], 2);
    }
...
//      
//      // Actuator constraints at time t-1
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (int i = 0; i < N-1; ++i) { // latency
    result.push_back(solution.x[x_start + i + 1]); // latency
    result.push_back(solution.x[y_start + i + 1]); // latency
  }
